...
C.log_dir = osp.abspath(osp.join(C.root_dir, 2 * '../' + 'models/TorchSeg/finetuned'))
...
C.dataset_path = osp.join(C.root_dir, 2 * '../', 'datasets/data_info')
C.img_root_folder = C.dataset_path
C.gt_root_folder = C.dataset_path
C.train_source = osp.join(C.dataset_path, "train.list")
C.eval_source = osp.join(C.dataset_path, "val.list")
C.test_source = osp.join(C.dataset_path, "test.list")
C.is_test = False
...
C.bn_eps = 1e-5
C.bn_momentum = 0.1
C.pretrained_model = osp.join(4 * '../', 'models/TorchSeg/pretrained/cityscapes-bisenet-R18.pth')
...
C.eval_base_size = 640
C.eval_crop_size = 640

"""Display Config"""
C.snapshot_iter = 5 ## Epoch Checkpoint
